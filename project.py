...
class Project:
    ...
    def __init__(self, name):
        ...
        self._project = {'Milestones': defaultdict(int), 'Components': defaultdict(
            int), 'Labels': defaultdict(int), 'Issues': []}
    ...
    def _append_item_to_project(self, item):
        ...
        # print("=======================")
        # print(self._project['Issues'])
        # print("-----------------------")

        # print(item.title.text[item.title.text.index("]") + 2:len(item.title.text)])

        # print("-----------------------")
        # print(item.description)
        # print("-----------------------")
        # print(self._htmlentitydecode(item.description.text))
        # print("-----------------------")

        # username = item.reporter.get('username')
        # print(item.reporter)
        # print(username)
        # print("-----------------------")

        if "".__eq__(item.description):
            description = item.title.text[0:item.title.text.index(
                "]") + 1] + ' created by ' + item.reporter + '</i>'
        else:
            description = self._htmlentitydecode(
                item.description.text) + '\n<i>' + item.title.text[0:item.title.text.index("]") + 1] + ' created by ' + item.reporter + '</i>'
